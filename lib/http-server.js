...
// main server code
function httpServer(workdir, config) {

    const logStatement = "Server started on port " + config['port'] + " http://localhost:" + config['port'] + "/";
    ...
    const reloadScript = "<script>const socket = new WebSocket(" + socketUrl + ");socket.addEventListener('open', function (event) {}); socket.addEventListener('message', function (event) {if  (event.data==='Reload'){ location.reload(); };});</script> </html>"
